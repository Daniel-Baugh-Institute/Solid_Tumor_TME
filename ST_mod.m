...
alpha=P(13);               % CAF barrier

K_CAFB=P(14);              % Barrier formation rate

K_CPDNPD= P(15);           % NPDL1 to PDL1 conversion

K_CNPDL1D=P(16);           % Death rate of NPDL1

K_CPDL1D=P(17);            % Death rate of PDL1

K_ResD=P(18);              % Degradation of Resources

Delta=P(19);               % Width of the barrier
...
K_CAFT=P(26);              % CAF-driven inhibition of T cells through regulatory T cells.

K_TKPDD=P(27);             % Death rate for killer PD1+ T cells

K_TEXD=P(28);              % Death rate for exhausted T cells

%% CAF proliferation, interactions, and death

Y_CAF=P(29);               % maximum carrying capacity of Fibroblasts

K_CAF=P(30);               % Proliferation rate of CAF cells 

K_CTCAF=P(31);             % Tumor cells-induced growth of CAF

K_CTCAFR=P(32);            % Proximity factor for CAF and resistant tumor cells  

K_M2CAF=P(33);             % M2-driven proliferation rate for CAFs 

K_CAFD=P(34);              % Death rate for CAF

%% Macrophages
Y_M= P(35);               % Carrying capacity

K_M1= P(36);               % Growth rate of M1 phase macrophage

K_CANM1= P(37);            % Tumor cells-driven proliferation of macrophages

K_M1M2=P(38);              % Change of polarity

K_M1D= P(39);              % Death rate of M1 phase macrophage 

K_M2= P(40);               % Growth rate of M2 phase macrophage

K_CAFM2= P(41);            % CAF-driven proliferation of M2 macrophages

alpha_TM= P(42);           % Functional approximation of ICAM1 wrt T cells

K_M2D= P(43);              % Death rate of M2 phase macrophage 

%% IL-8 Secretion
K_M2IL8=P(44);             % IL8 secretion by M2

K_CAFIL8=P(45);            % IL8 secretion by CAF

K_CANIL8=P(46);            % IL8 secretion by tumor cells

K_IL8D=P(47);              % Degradation rate of IL8
...
F_ResCNPDL1=Res*K_RCNPDL1*CNPDL1*(1-CNPDL1/(Y_CNPDL1M*(1-I)+1));                                  % Resource-driven proliferation of tumor stem cells
...
F_ResCPDL1=Res*K_RCPDL1*CPDL1*(1-CPDL1/(Y_CPDL1M*(1-I)+0.01));                                     % Resource-driven proliferation of PDL1 +ve cells
...
F_ResCRNPDL1=Res*K_RCRNPDL1*CRNPDL1*(1-CRNPDL1/(Y_CNPDL1M*I+0.01));                                 % Resource-driven proliferation of tumor stem cells

F_CAFCRNPDL1=(1+K_CAFCR*CAF/(CAF+1));                                                                                    % CAF-driven proliferation of the tumor cells

F_CRNPDL1CRPDL1= K_CPDNPD*CRNPDL1*alpha_T+2.5*CRNPDL1;                                                                                       % Conversion from stem to PDL1, T-exposed tumor cells

F_TxCR=(1+K_TXC*TEX/(1+TEX)*alpha_T);                                                                                    % Exhausted T-cells-driven proliferation modulator of tumor stem cells

F_TKCRNPDL1= K_TKC*CRNPDL1*(TKPD1+TKNPD1)*alpha_T;                                                                       % T-cell driven apoptosis of the stem cell

F_DCRNPDL1= (K_CNPDL1D+M1/(M1+1))*CRNPDL1;                                                                                           % Death of stem cells

dydt_CRNPDL1=F_ResCRNPDL1*F_CAFCRNPDL1*F_TxCR-F_TKCRNPDL1-F_CRNPDL1CRPDL1-F_DCRNPDL1;

% PDL1+ve Non-T-exposed tumor stem cell

F_ResCRPDL1=Res*K_RCRPDL1*CRPDL1*(1-CRPDL1/(Y_CPDL1M*I+0.01));                                    % Resource-driven proliferation of tumor stem cells

F_TKCRPDL1= K_TKC*CRPDL1*(TKNPD1)*alpha_T;                                                                              % T-cell driven apoptosis of the stem cell

F_DCRPDL1= (K_CNPDL1D+M1/(M1+1))*CRPDL1;                                                                                            % Death of stem cells
...
F_ConsRes=Res*(K_RCNPDL1*(CNPDL1+CPDL1)+K_RCPDL1*(CRPDL1+CRNPDL1));
...
F_CANCAF=(1+ K_CTCAF*(CPDL1+CNPDL1)/(1+CPDL1+CNPDL1));                                                                                     % ICI-based conversion from PD1+ve to -ve killer T cells

F_CANRCAF=(1+ K_CTCAFR*(CRPDL1+CRNPDL1)/(1+CRPDL1+CRNPDL1));                                                                       % ICI-based conversion from PD1+ve to -ve killer T cells

F_M2CAF=(1+K_M2CAF*M2/(M2+1));

F_DCAF = K_CAFD*CAF;                                                                                                                        % Death rate of CAFs

dydt_CAF = F_ProCAF*F_CANCAF*F_CANRCAF*F_M2CAF-F_DCAF; 
...
dydt = [dydt_CNPDL1;dydt_CPDL1;dydt_CRNPDL1;dydt_CRPDL1;dydt_Res;dydt_TKPD1;dydt_TKNPD1;dydt_TEX;dydt_CAF;dydt_M1;dydt_M2;dydt_IL8];
